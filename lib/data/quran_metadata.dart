...
/// 240 (surah, ayah) entries marking every quarter of every hizb, in order.
/// Index i: i%4==0 -> hizb start (i%8==0 = juz start); 1 -> 1/4,
/// 2 -> 1/2, 3 -> 3/4. Source: alquran.cloud /v1/meta hizbQuarters.
const List<(int, int)> kHizbQuarterStarts = <(int, int)>[
  (1, 1),
  (2, 26),
  (2, 44),
  (2, 60),
...
/// Returns the index 0..239 of (surah, ayah) in [kHizbQuarterStarts], or
/// null if it is not the start of any hizb quarter. Index%4 selects the
/// quarter: 0 = hizb start, 1 = 1/4, 2 = 1/2, 3 = 3/4.
int? hizbQuarterIndex(int surah, int ayah) {
  final i = kHizbQuarterStarts.indexOf((surah, ayah));
  return i == -1 ? null : i;
}

/// Returns the 1-based ruku number within [surah] of (surah, ayah),
/// or null if (surah, ayah) is not the start of any ruku in that surah.
int? rukuNumberInSurah(int surah, int ayah) {
  var n = 0;
  for (final r in kRukuStarts) {
    final (s, _) = r;
    if (s == surah) n++;
    if (r == (surah, ayah)) return n;
  }
  return null;
}
